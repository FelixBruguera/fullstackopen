    ...
    describe('when logged in', () => {
        ...
        describe('with some blogs in the database', async (page) => {
            ...
            test('a blog can be liked', async ({ page }) => {
                const blog = await page.locator('.blog').filter({ hasText: 'Second Blog' })
                blog.locator('button').click()
                await blog.getByRole('button', { name: 'Like' }).click()
                await expect(blog.locator('.blog-likes')).toContainText('Likes: 1')
            })
            test('the user who added a blog can delete it', async ({ page }) => {
                ...
                await expect(page.locator('.blog').filter({ hasText: 'Loren Ipsum'})).not.toBeVisible()
            ...
            test('the blogs are sorted by likes', async({ page }) => {
                ...
                await expect(page.locator('.blog-likes')).toHaveText('Likes: 0')
            })
        })
    })
